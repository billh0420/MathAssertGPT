...
# (nm 27-Jun-2005) mmverify.py requires that an $f hypothesis must not occur
# after an $e hypothesis in the same scope, even though this is allowed by
...
class MM:
    ...
    def decompress_proof(self, statement_label, stat, proof):
        ...
        subproofs = []
        prev_proofs = []
        for index, pf_int in enumerate(proof_ints):
            # if statement_label == 'f1oweALT':
            #     print(f'{statement_label}: index={index}')
            if pf_int == -1:
                ...
                if step_type in ('$a', '$p'):
                    ...
                    if nshyps != 0:
                        new_prevpf = [s for p in prev_proofs[-nshyps:] for s in p] + [pf_int]
                        prev_proofs = prev_proofs[:-nshyps]
                        if statement_label == 'f1oweALT' and len(new_prevpf) > 20000:
                            print(f'{index}. {statement_label}: pf_int={pf_int} #prev_proofs={len(prev_proofs)} #new_prevpf={len(new_prevpf)}')
        ...
        for label in proof:
            ...
            if steptyp in ('$a', '$p'):
                ...
                for (k, v) in mand_var:
                    entry = stack[sp]
                    if entry[0] != k:
                        raise MMError(("stack entry ({0}, {1}) doesn't match " + "mandatory var hyp {2!s}").format(k, v, entry))
                    subst[v] = entry[1:]
                    sp += 1
                ...
                for x, y in distinct:
                    ...
                    for x1, y1 in itertools.product(x_vars, y_vars):
                        if x1 == y1 or not self.fs.lookup_d(x1, y1):
                            raise MMError(f"disjoint violation: {x1}, {y1}")
                for h in hyp:
                    entry = stack[sp]
                    subst_h = apply_subst(h, subst)
                    if entry != subst_h:
                        raise MMError(("stack entry {0!s} doesn't match " + "hypothesis {1!s}").format(entry, subst_h))
                    sp += 1
...
if __name__ == '__main__':
    print("Start")
    sys.stdin = open("set.new2023.mmx", "r")
    mm = MM()
